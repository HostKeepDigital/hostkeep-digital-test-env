...
export default function PropertyListingCard({ 
  ...
  if (property.amenities?.length > 0) score += 20;
  if (property.nightly_rate > 0) score += 20;
  if (property.ical_url || property.blocked_dates?.length > 0 || property.day_based_restrictions_enabled) score += 20;
  ...
  const hasHouseRules = property.house_rules && property.house_rules.length > 5;
  const hasPricingRules = property.pricing_settings?.weekend_rate || property.pricing_settings?.seasons?.length > 0;

  // Find upcoming bookings without cleaning jobs
  const propertyBookings = upcomingBookings?.filter(b => b.property_id === property.id && b.check_in && isAfter(parseISO(b.check_in), new Date())) || [];
  const missingCleaners = propertyBookings.filter(b => !cleaningJobs?.some(cj => cj.booking_id === b.id && cj.status !== 'cancelled' && cj.status !== 'declined'));
  
  return (
    <div className={`group relative bg-white rounded-[20px] border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col ${isSingle ? 'max-w-xl w-full' : 'w-full'}`}>
      {/* Image Header */}
      <div className="relative h-72 overflow-hidden bg-gray-100">
        ...
        <Badge className={`absolute top-4 left-4 font-semibold border-0 shadow-sm px-3 py-1 ${statusColors[property.status]}`}>
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 translate-x-12 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
          <Link to={createPageUrl('EditProperty') + `?id=${property.id}&tab=basics`}>
            <Button size="icon" variant="secondary" className="h-10 w-10 rounded-xl shadow-md bg-white/95 hover:bg-white text-gray-700"><Edit3 className="w-4 h-4" /></Button>
          </Link>
          <Link to={createPageUrl('EditProperty') + `?id=${property.id}&tab=photos`}>
            <Button size="icon" variant="secondary" className="h-10 w-10 rounded-xl shadow-md bg-white/95 hover:bg-white text-gray-700"><ImageIcon className="w-4 h-4" /></Button>
          </Link>
      ...
      <div className="p-6 flex-1 flex flex-col">
        {missingCleaners.length > 0 && (
          <div className="mb-6 p-4 rounded-xl bg-rose-50 border border-rose-100 flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-rose-900">Missing Cleaner Assignments</p>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-3 mb-7">
          <div className="flex items-center gap-2 text-sm">
            {hasCleaningTeam ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <AlertTriangle className="w-4 h-4 text-amber-500" />}
            <span className={hasCleaningTeam ? "text-gray-700" : "text-gray-500"}>
              {hasCleaningTeam ? "Cleaning Team" : "No Cleaner"}
            </span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            {hasHouseRules ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <AlertTriangle className="w-4 h-4 text-amber-500" />}
            <span className={hasHouseRules ? "text-gray-700" : "text-gray-500"}>
              {hasHouseRules ? "House Rules" : "No House Rules"}
            </span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            {hasPricingRules ? <CheckCircle2 className="w-4 h-4 text-emerald-500" /> : <CheckCircle2 className="w-4 h-4 text-gray-400" />}
            <span className={hasPricingRules ? "text-gray-700" : "text-gray-500"}>
        ...
        <div className="mb-6 bg-gray-50/50 p-4 rounded-xl border border-gray-100">
          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Settings Categories</h4>
          <div className="space-y-2.5">
            <div className="flex justify-between text-sm">
              <span className="text-gray-600 font-medium">Photos</span>
              <span className="text-gray-500">{property.photos?.length || 0} uploaded</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600 font-medium">Amenities</span>
              <span className="text-gray-500">{property.amenities?.length || 0} selected</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600 font-medium">House Rules</span>
              <span className="text-gray-500">{hasHouseRules ? 'Configured' : 'Not Set'}</span>
            </div>
            <div className="flex justify-between text-sm">
