...
export default function BalancePaymentAlert({
  ...
  const isSuperStrict = booking.cancellation_policy_snapshot?.type === "Super Strict";
  const depositRefund = isSuperStrict ? (booking.security_deposit || 0) * 0.5 : 0;
...
// ── Modal — fetches key and provides Elements wrapper ─────────────────────────
function BalancePaymentModal({ booking, property, balanceOwed, onClose, onSuccess }) {
...
function PaymentForm({ booking, property, balanceOwed, onClose, onSuccess }) {
  ...
  if (paymentSucceeded) {
    return (
      <div className="p-6 text-center">
        ...
        <h2 className="text-lg font-semibold text-gray-900 mb-2">Payment received</h2>
    ...
    <form onSubmit={handleSubmit} className="p-6 space-y-4">
      ...
      <div className="border border-gray-300 rounded-lg p-3 bg-white">
        <CardElement
          options={{
            style: {
              base: { fontSize: "14px", color: "#1E3A5F", "::placeholder": { color: "#999" } },
