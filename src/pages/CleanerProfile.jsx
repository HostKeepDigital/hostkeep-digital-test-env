...
export default function CleanerProfile() {
  ...
  const { data: reviews = [] } = useQuery({
    ...
    queryFn: () => base44.entities.CleanerReview.filter({ cleaner_id: cleanerId, visible: true }),
    enabled: !!cleanerId,
  });
  ...
  const avgQuality = reviews.length > 0
    ? (reviews.reduce((sum, r) => sum + (r.quality_rating || 0), 0) / reviews.filter(r => r.quality_rating).length).toFixed(1)
    : 0;

  const avgReliability = reviews.length > 0
    ? (reviews.reduce((sum, r) => sum + (r.reliability_rating || 0), 0) / reviews.filter(r => r.reliability_rating).length).toFixed(1)
    : 0;

  const avgCommunication = reviews.length > 0
    ? (reviews.reduce((sum, r) => sum + (r.communication_rating || 0), 0) / reviews.filter(r => r.communication_rating).length).toFixed(1)
    : 0;
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4 mb-4">
                {cleaner.average_rating > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-semibold">{cleaner.average_rating.toFixed(1)}</span>
                    <span className="text-teal-100">({cleaner.total_reviews} reviews)</span>
                  </div>
                )}
              ...
              {cleaner.service_area?.city && (
                <div className="flex items-center gap-2 text-teal-100 mb-4">
                  ...
                  <span>Services {cleaner.service_area.city} • {cleaner.service_area.radius_miles} mile radius</span>
                </div>
              )}
              ...
              <div className="flex gap-3">
                <Dialog open={showAssignDialog} onOpenChange={setShowAssignDialog}>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Assign {cleaner.business_name} to Property</DialogTitle>
                    </DialogHeader>
              ...
              <div className="grid md:grid-cols-4 gap-4">
                ...
                <Card>
                  <CardContent className="p-4">
                    ...
                    <div className="text-2xl font-bold text-gray-900">{cleaner.acceptance_rate}%</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4">
                    <div className="text-sm text-gray-500 mb-1">Avg Duration</div>
                    <div className="text-2xl font-bold text-gray-900">{cleaner.average_duration_hours}h</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardContent className="p-4">
                    <div className="text-sm text-gray-500 mb-1">Max Jobs/Day</div>
                    <div className="text-2xl font-bold text-gray-900">{cleaner.max_jobs_per_day}</div>
                  </CardContent>
                </Card>
              </div>
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={cleaner.verified ? "text-gray-900" : "text-gray-500"}>
                        Identity Verified
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <span className={cleaner.insurance_verified ? "text-gray-900" : "text-gray-500"}>
                        Insurance Verified
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-sm text-gray-600 mb-1">Per Bedroom</div>
                      <div className="text-xl font-semibold text-gray-900">
                        +£{cleaner.price_per_bedroom || 0}
                      </div>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-4">
                      <div className="text-sm text-gray-600 mb-1">Per Bathroom</div>
                      <div className="text-xl font-semibold text-gray-900">
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="text-sm text-blue-700 mb-1">Minimum Charge</div>
                    <div className="text-xl font-semibold text-blue-900">£{cleaner.minimum_charge}</div>
                  </div>

                  <div className="text-sm text-gray-500 p-4 bg-gray-50 rounded-lg">
                    <strong>Example:</strong> A 3-bedroom, 2-bathroom property would cost approximately:
                    <div className="mt-2 text-gray-900 font-medium">
                      £{cleaner.base_price} + (3 × £{cleaner.price_per_bedroom || 0}) + (2 × £{cleaner.price_per_bathroom || 0}) 
                      = £{cleaner.base_price + (3 * (cleaner.price_per_bedroom || 0)) + (2 * (cleaner.price_per_bathroom || 0))}
                    </div>
                  </div>
                </div>
                  ...
                  <CardContent className="p-6">
                    <div className="grid md:grid-cols-4 gap-4">
                      <div className="text-center">
                        ...
                        <div className="flex items-center justify-center gap-1 mb-2">
                          {[1, 2, 3, 4, 5].map(i => (
                            <Star 
                              key={i} 
                              className={`w-4 h-4 ${i <= cleaner.average_rating ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}`} 
                            />
                          ))}
                        </div>
                      ...
                      <div className="bg-gray-50 rounded-lg p-3">
                        ...
                        <div className="text-2xl font-bold text-gray-900">{avgReliability}</div>
                      </div>
                      <div className="bg-gray-50 rounded-lg p-3">
                        <div className="text-xs text-gray-500 mb-1">Communication</div>
                        <div className="text-2xl font-bold text-gray-900">{avgCommunication}</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                    ...
                    <CardContent className="p-6">
                      ...
                      {review.cleaner_response && (
                        <div className="bg-gray-50 rounded-lg p-4 mt-3 border-l-4 border-blue-500">
                          ...
                          <p className="text-sm text-gray-600">{review.cleaner_response}</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
