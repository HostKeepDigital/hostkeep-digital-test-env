...
import { 
  ...
  MapPin, Users, Bed, Star, Home, Crown, X, PlusCircle
} from "lucide-react";
...
export default function HostProperties() {
  ...
  const { data: cleanerSettings = [] } = useQuery({
    ...
    queryFn: () => base44.entities.PropertyCleanerSettings.filter({ host_id: user?.id }),
    enabled: !!user?.id,
  });

  const { data: upcomingBookings = [] } = useQuery({
    queryKey: ['upcoming-bookings', user?.id],
    queryFn: () => base44.entities.Booking.filter({ host_id: user?.id, booking_status: 'confirmed' }),
    enabled: !!user?.id,
  });
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        {!user || isLoading ? (
          ...
          >
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto">Start your hosting journey by adding your first property. It only takes a few minutes to get setup.</p>
          ...
          <div className="space-y-12">
            <div className={properties.length === 1 ? "flex justify-center" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"}>
              {properties.map((property, idx) => (
                <motion.div
                  ...
                  className={properties.length === 1 ? "w-full flex justify-center" : ""}
                >
                  <PropertyListingCard 
                    property={property}
                    cleanerSettings={cleanerSettings.find(cs => cs.property_id === property.id)}
            ...
            {/* Empty State / Upsell Footer for single property */}
            {properties.length === 1 && (
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <div className="inline-flex flex-col items-center p-8 rounded-3xl bg-gray-50/80 border border-gray-100">
                  ...
                  <h4 className="text-lg font-semibold text-gray-900 mb-2">Thinking of expanding?</h4>
                  <p className="text-gray-500 mb-6 text-sm max-w-sm">Grow your hosting business by adding another property to your portfolio.</p>
                  <Button 
                    onClick={handleAddPropertyClick}
                    variant="outline"
