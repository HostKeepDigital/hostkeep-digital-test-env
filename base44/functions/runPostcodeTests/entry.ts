...
async function lookupPlace(query) {
  const res = await fetch(`https://api.postcodes.io/places/${encodeURIComponent(query)}`);
  return res.json();
}
...
async function runSuite() {
  ...
  for (let i = 0; i < BATCH_COUNT; i++) {
    ...
    try {
      log(`🎯 [Batch ${batch.index}] Region=${batch.region} Postcode=${batch.postcode} Radius=${batch.radiusMiles}mi`);

      // 1) Full lookup
      const lookup = await lookupPostcode(batch.postcode);
      batch.steps.push({ name: "Full Lookup", status: lookup?.status });
      if (lookup?.status !== 200) throw new Error(`Lookup failed: ${batch.postcode} (${lookup?.status})`);
      const { latitude, longitude } = lookup.result || {};
      if (typeof latitude !== "number" || typeof longitude !== "number") throw new Error("Missing coordinates");
      pass("Full Lookup", { batch: batch.index, postcode: batch.postcode });

      // 2) Partial search
      const search = await searchPartial(batch.outward);
      batch.steps.push({ name: "Partial Search", outward: batch.outward, count: search?.result?.length || 0 });
      if (search?.status !== 200 || !Array.isArray(search?.result) || search.result.length === 0)
        throw new Error(`Partial search empty for ${batch.outward}`);
      pass("Partial Search", { batch: batch.index, outward: batch.outward });
      ...
      if (place?.status === 200) {
        pass("Place Lookup", { batch: batch.index, place: placeName });
      } else {
        warn("Place Lookup", { batch: batch.index, place: placeName, note: "404 acceptable for some places" });
      }

      // 4) Radius smoke test
      const props = [
        { id: 1, lat: latitude + 0.02, lng: longitude + 0.02 },
        { id: 2, lat: latitude + 1.00, lng: longitude + 1.00 }
      ];
      const inside = props.filter(p => haversineMiles(latitude, longitude, p.lat, p.lng) <= batch.radiusMiles);
      batch.steps.push({ name: "Radius", radiusMiles: batch.radiusMiles, insideIds: inside.map(p => p.id) });
      if (!inside.some(p => p.id === 1) || inside.some(p => p.id === 2))
        throw new Error(`Radius expectations failed @ ${batch.radiusMiles}mi`);
      pass("Radius Math", { batch: batch.index, radiusMiles: batch.radiusMiles });

    } catch (e) {
      batch.ok = false;
      fail("Batch", e, { batch: batch.index, postcode: batch.postcode });
    }

    batches.push(batch);
  }

  const ok = summary.every(s => s.status === "PASS" || s.status === "WARN");
...
function buildEmailBody({ ok, batches, summary }) {
  ...
  for (const s of summary) {
    ...
    lines.push(`  [${s.status}] ${s.section}${parts.length ? " (" + parts.join(", ") + ")" : ""}`);
  }
  return lines.join("\n");
}
  ...
  try {
    ...
    try {
      ...
      await base44.integrations.Core.SendEmail({
        ...
        body: `The postcode test endpoint threw an unexpected error:\n\n${error.message}\n\n${error.stack || ""}`
      });
    } catch (_) {}
