...
Deno.serve(async (req) => {
  ...
  // UPDATED: Use the new onboarding approval function
  const approveFunctionUrl =
    "https://api.base44.app/api/apps/698eee4108bd1d9467648326/functions/promoteUserToInvited";
  ...
  await base44.asServiceRole.integrations.Core.SendEmail({
    ...
    body: `
      <div style="font-family:sans-serif;max-width:560px;margin:0 auto;">
        <h2 style="color:#0d9488;">New ${
          role.charAt(0).toUpperCase() + role.slice(1)
        } Application</h2>

        <table style="border-collapse:collapse;width:100%;margin-bottom:24px;">
          <tr>
            <td style="padding:8px;color:#6b7280;width:120px;">Name</td>
          ...
          <tr>
            <td style="padding:8px;color:#6b7280;">Role applied for</td>
            <td style="padding:8px;text-transform:capitalize;">${role}</td>
          </tr>
          <tr style="background:#f9fafb;">
            <td style="padding:8px;color:#6b7280;">Registered</td>
        ...
        <a href="${approveFunctionUrl}"
           onclick="fetch('${approveFunctionUrl}',{
             method:'POST',
             headers:{'Content-Type':'application/json'},
             body:JSON.stringify({ role_id: '${roleRecord?.id}' })
           });
